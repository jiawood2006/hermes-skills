...
| 路径 | 效果 | 何时用 |
|---|---|---|
| **① 产品锁 + `wan3.0-video`** ✅ **当前主线** | 同 prompt 同 5 张参考图下产品**一次就对、整段不掉金**；真人生活情景。参考图=刀网微距/正面全身/真手比例/底部端面/男主 | 绝大多数带货片（有官方产品图、要求"产品像真机"）|
| **② 官方真像素产品镜（0 API 成本）** ✅ **产品保真终极解** | 产品 **100% 等于真机**（官方真像素，不经过 AI）| AI 总把产品画不像时；或片子里要让人一眼认出产品 |
| ③ 文生视频（`wan2.6-t2v`，台词走 `input.audio_url`）| 真人 + 场景 + 中文口型，但**产品每段会变一点** | 无官方素材、或纯人物剧情 |
...
**铁律**：
- ❌ `wan2.7-r2v` 做产品镜**已退役**（产品小/看不清/易洗成银白 = 反复返工根因），只留人物戏
- **生成一律 720P**（原生 1080P 生成 = 重抽卡不是升级，实测产品变形/掉金/并不更锐）→ 成片由装配升 1080
- `wan3.0-video` 会把**参考图1 当画面首帧**（要控开场就换图1）
- 用户给的"样片标准"若是真人剧情对话，**不要断言"只能实拍"**（用户明确纠正过：他的样片本身就是 AI 生成的）
