...
| 15 | 高频 AI 词（互联网黑话见 23） | 赋能／闭环／抓手／底层逻辑／生态位 | 换成具体动作和数字 |
...
**AI味指数怎么算（确定公式 · 改写前后必须用同一个）**：

```
命中处数 = 每条规则的每一处正则命中各计 1 处（同一模式重复出现会累计）

每千字命中数 = 命中处数 / max(1, 总字数 / 1000)
指数 = min(100, int(28 + 每千字命中数 × 14))      # 0 命中 = 基线 28；约 5 处/千字 ≈ 98
```

- 分级（与脚本输出一致）：**≥70「AI味重，一眼假」· 45–69「有AI味，需要改」· <45「比较像人写的」**。
- 这是基于命中密度的启发式分数（实现见 `scripts/demo.py: ai_score()`），**不是 AI 检测器**——不能判定"这段是不是 AI 写的"；逐条模式无法判定时只报命中处数，不给分。
- 改写后重算必须用同一公式，分数差才有意义。
